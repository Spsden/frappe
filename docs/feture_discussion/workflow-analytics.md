...
| 6–14 | Preliminary averages and path differences |
...
- Require at least 15 recordings before attempting path clustering.
- Prefer 30 or more recordings before presenting clusters confidently to executives.
- Below the clustering threshold, provide descriptive comparisons without claiming statistically reliable execution-path clusters.
...
## Future full workflow analytics

Once enough recordings are available, the workflow-level dashboard can add:

- Session-level execution-path clustering
- Two to four path clusters selected using a quality score
- Cluster prevalence percentages
- Per-step friction scores
- Best successful cohort versus average cohort
- Trends over time
- Department or role comparisons where privacy thresholds permit them

This later dashboard should operate on stable, persisted analytics snapshots generated asynchronously. Dashboard requests should read the latest completed snapshot rather than recalculating embeddings or clusters on demand.

## Current decisions

- Build recording comparison before full workflow clustering.
...
- Analyze all eligible recordings for future workflow-level analytics.
- Do not present K-means clustering as reliable with fewer than 15 recordings.
- Do not label the fastest recording as a top performer without a success or quality signal.

## Open questions

- How will a recording be marked as successfully completed?
- Will workflows have canonical steps, or will the first approved SOP become the initial canonical sequence?
- How should unmatched and optional steps be represented during alignment?
- Should analytics runs be saved permanently, versioned, or regenerated on demand?
- Should users be able to choose a date range for full workflow analytics?
- What role or permission is required to view employee-linked comparisons?
